...
//   private BeanWrapper beanWrapper = new BeanWrapperImpl(TrackDTO.builder().build());
   ...
   public TrackService(TrackDAO trackDAO, PricingProvider pricingProder) {
...
//      tracks.forEach(this::addPriceToTrack);
   ...
   public List<TrackDTO> getTracksBySearchSpec(Map<String, String> rawQueryStrings) {
      Map<String, Object> queryStrings = convertToMapOfCorrectTypes(rawQueryStrings);
      ...
      queryStrings.forEach((key, value) -> {
         var ss = switch (key) {
            case String s when s.equals("artist") -> new InMemorySearchSpec<TrackDTO>(
                  InMemorySearchSpec.SearchType.Custom,
                  "artist", value,
                  (t) ->
            ...
            case String s when (s.equals("title") || s.equals("album")) ->
                  new InMemorySearchSpec<TrackDTO>(InMemorySearchSpec.SearchType.ContainsString,
                        key, value);
            default -> null;
         };
         if (ss != null) {

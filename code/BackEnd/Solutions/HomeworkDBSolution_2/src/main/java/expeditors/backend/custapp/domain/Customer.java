...
public class Customer implements Comparable<Customer> {
    ...
    @OneToMany(cascade = CascadeType.ALL,
            mappedBy = "customer",
            orphanRemoval = true)
    //Need to add a join column if you make this a one way relationship.
    //See comments in PhoneNumber for more detail.
...
//        this.phoneNumbers.addAll(phoneNumbers);
    ...
    public Customer(int id, String name, LocalDate dob, Status status,
        ...
        this(id, name, dob, status, Set.of(phoneNumbers));
    }

    public Customer(int id, String name, LocalDate dob, Status status,
                    List<PhoneNumber> phoneNumbers) {
        this(id, name, dob, status, Set.copyOf(phoneNumbers));
    }

    public Customer(String name, LocalDate dob, Status status, PhoneNumber... phoneNumbers) {
        this(0, name, dob, status, Set.of(phoneNumbers));
    }

...
@Service
public class JPASearchSpecServicePlusPlus<E_T, ID_T,
      RepoType extends JpaRepository<E_T, ID_T> & JpaSpecificationExecutor<E_T>> {

      Function<Pageable, Page<E_T>> findAllWithPageable = null;
   Supplier<List<E_T>> findAllSimple = null;
      BiFunction<Specification<E_T>, Pageable, Page<E_T>> findAllWithSpecAndPage =  null;
      Function<Specification<E_T>, List<E_T>> findAllWithSpec = null;


   public ResultWithPageData<E_T> doSearch(@RequestParam Map<String, Object> queryStrings,
                                                RepoType targetRepository) {

//      findAllWithPageable = findAllWithPageable == null ?
//            targetRepository::findAll : findAllWithPageable;
//      findAllSimple =  findAllSimple == null ?
//            targetRepository::findAll : findAllSimple;
      findAllWithSpecAndPage =  findAllWithSpecAndPage == null ?
            targetRepository::findAll : findAllWithSpecAndPage;
      findAllWithSpec =  findAllWithSpec == null ?
            targetRepository::findAll : findAllWithSpec;
...
//            .stream().collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
      ...
      if (haveSearchParams.isPresent()) {
         ...
         Page<E_T> page = findAllWithPageable.apply(pageable);
...
//         var page = specExecutor.findAll(specification, pageAble);
         var page = findAllWithSpecAndPage.apply(specification, pageAble);
         searchContext.put("totalElements", page.getTotalElements());
         searchContext.put("totalPages", page.getTotalPages());
         result = page.getContent();
      } else {
         //result = specExecutor.findAll(specification);
         result = findAllWithSpec.apply(specification);
      }
...
//      Map<String, Object> result = new HashMap<>();
      searchContext.forEach((k, v) -> {
         switch (k) {
            case String s when
                  s.contains("adoptionDate") -> searchContext.put(k, LocalDate.parse(v.toString()));

            case String s when
                  s.contains("type") -> searchContext.put(k, PetType.valueOf(v.toString().toUpperCase()));

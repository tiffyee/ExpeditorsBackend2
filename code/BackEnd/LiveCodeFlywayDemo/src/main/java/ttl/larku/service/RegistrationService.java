...
public class RegistrationService {
    ...
    public boolean deleteClassFromSchedule(String courseCode, LocalDate startDate, LocalDate endDate) {
        List<ScheduledClass> sClass = classService.getScheduledClasses(courseCode, startDate, endDate);
        sClass.forEach(this::deleteClassFromSchedule);

        return !sClass.isEmpty();
    }

    public boolean deleteClassFromSchedule(ScheduledClass sc) {
       sc.getStudents()
             .forEach(s -> {
                 s.dropClass(sc);
             });

       return classService.deleteScheduledClass(sc.getId());
    }
